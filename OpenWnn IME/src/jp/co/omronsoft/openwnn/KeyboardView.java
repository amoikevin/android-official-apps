...
public class KeyboardView extends View implements View.OnClickListener {
	...
	private final int mKeyTextColor2nd;
	...
	private final Drawable mKeyBackground2nd;
	...
	Handler mHandler = new Handler() {
		...
		public void handleMessage(final Message msg) {
			...
			case MSG_REPEAT:
				if (repeatKey()) {
					final Message repeat = Message.obtain(this, MSG_REPEAT);
	...
	public KeyboardView(final Context context, final AttributeSet attrs, final int defStyle) {
		...
		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.KeyboardView, defStyle,
				R.style.WnnKeyboardView);

		final LayoutInflater inflate = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);

		int previewLayout = 0;
		final int keyTextSize = 0;
		...
		a.recycle();
		// a = context.obtainStyledAttributes(android.R.styleable.Theme);
		// mBackgroundDimAmount =
		// a.getFloat(android.R.attr.Theme_backgroundDimAmount, 0.5f);
	...
	private void initGestureDetector() {
		mGestureDetector = new GestureDetector(getContext(), new GestureDetector.SimpleOnGestureListener() {
			...
			public boolean onFling(final MotionEvent me1, final MotionEvent me2, final float velocityX,
				...
				final float deltaX = me2.getX() - me1.getX();
				final float deltaY = me2.getY() - me1.getY();
				final int travelX = getWidth() / 2;
				final int travelY = getHeight() / 2;
	 ...
	 *
	 * @param listener
	 *            The OnKeyboardActionListener to set.
	 */
	...
	public void setKeyboard(final Keyboard keyboard) {
		...
		if (oldRepeatKeyCode != NOT_A_KEY) {
			final int keyIndex = getKeyIndices(mStartX, mStartY, null);
			if (keyIndex != NOT_A_KEY && keyIndex < mKeys.length && oldRepeatKeyCode == mKeys[keyIndex].codes[0]) {
				abort = false;
	...
	public boolean isParentPreviewEnabled() {
		if (mPopupParent != null && mPopupParent != this && mPopupParent instanceof KeyboardView)
	 ...
	 * @param keyboard
	 */
	private void computeProximityThreshold(final Keyboard keyboard) {
		if (keyboard == null)
			return;
	...
	private void onBufferDraw() {
		final boolean isBufferNull = mBuffer == null;
		if (isBufferNull || mKeyboardChanged) {
			if (isBufferNull
					|| mKeyboardChanged && (mBuffer.getWidth() != getWidth() || mBuffer.getHeight() != getHeight())) {
		...
		for (int i = 0; i < keyCount; i++) {
			...
			final Drawable keyBackground = key.isSecondKey ? mKeyBackground2nd : mKeyBackground;
			...
			if (label != null) {
				...
				if (OpenWnn.isXLarge())
					canvas.drawText(label, (key.width - padding.left + 7 - padding.right) / 2 + padding.left,
				...
				int drawableY;
				if (OpenWnn.isXLarge()) {
					drawableX = (key.width - padding.left + 12 - padding.right - key.icon.getIntrinsicWidth()) / 2
							+ padding.left;
					drawableY = (key.height - padding.top + 9 - padding.bottom - key.icon.getIntrinsicHeight()) / 2
							+ padding.top;
				} else {
					drawableX = (key.width - padding.left - padding.right - key.icon.getIntrinsicWidth()) / 2
							+ padding.left;
					drawableY = (key.height - padding.top - padding.bottom - key.icon.getIntrinsicHeight()) / 2
							+ padding.top;
				}
		...
		if (mMiniKeyboardOnScreen) {
			paint.setColor((int) (1 * 0xFF) << 24);//mBackgroundDimAmount
	...
	private int getKeyIndices(final int x, final int y, final int[] allKeys) {
		...
		for (int i = 0; i < keyCount; i++) {
			...
			int dist = 0;
			final boolean isInside = key.isInside(x, y);
			if (isInside)
	 ...
	 * multi-tap state.
	 */
	private CharSequence getPreviewText(final Key key) {
		if (mInMultiTap) {
	...
	private void showKey(final int keyIndex) {
		...
		final int popupWidth = Math.max(mPreviewText.getMeasuredWidth(),
				key.width + mPreviewText.getPaddingLeft() + mPreviewText.getPaddingRight());
		final int popupHeight = mPreviewHeight;
		final LayoutParams lp = mPreviewText.getLayoutParams();
		if (lp != null) {
			lp.width = popupWidth;
		...
		if (mOffsetInWindow == null) {
			...
			final int[] mWindowLocation = new int[2];
	...
	public void invalidateKey(final int keyIndex) {
		...
		onBufferDraw();
		invalidate(key.x + getPaddingLeft(), key.y + getPaddingTop(), key.x + key.width + getPaddingLeft(),
				key.y + key.height + getPaddingTop());
	}
	...
	private boolean openPopupIfRequired(final MotionEvent me) {
		...
		final Key popupKey = mKeys[mCurrentKey];
		final boolean result = onLongPress(popupKey);
		if (result) {
			mAbortKey = true;
			showPreview(NOT_A_KEY);
		}
	...
	protected boolean onLongPress(final Key popupKey) {
		if (mKeyboardActionListener.onLongPress(popupKey))
			return true;
		final int popupKeyboardId = popupKey.popupResId;
		if (popupKeyboardId != 0) {
			...
			if (mMiniKeyboardContainer == null) {
				...
				final View closeButton = mMiniKeyboardContainer.findViewById(R.id.closeButton);
				if (closeButton != null)
				...
				if (popupKey.popupCharacters != null)
					keyboard = new Keyboard(getContext(), popupKeyboardId, popupKey.popupCharacters, -1,
	...
	public boolean onTouchEvent(final MotionEvent me) {
		...
		if (pointerCount != mOldPointerCount) {
			if (isPointerCountOne) {
				final MotionEvent down = MotionEvent.obtain(now, now, MotionEvent.ACTION_DOWN, me.getX(), me.getY(),
						me.getMetaState());
				...
				final MotionEvent up = MotionEvent.obtain(now, now, MotionEvent.ACTION_UP, mOldPointerX, mOldPointerY,
						me.getMetaState());
				result = onModifiedTouchEvent(up, true);
				up.recycle();
			}
		} else if (isPointerCountOne) {
	...
	private void checkMultiTap(final long eventTime, final int keyIndex) {
		if (keyIndex == NOT_A_KEY)
			return;
		final Key key = mKeys[keyIndex];
	...
	private static class SwipeTracker {
		...
		public void addMovement(final MotionEvent ev) {
