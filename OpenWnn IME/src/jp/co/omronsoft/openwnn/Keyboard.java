...
public class Keyboard {
	...
	public static class Row {
		...
		public Row(final Resources res, final Keyboard parent, final XmlResourceParser parser) {
			...
			defaultWidth = getDimensionOrFraction(a, R.styleable.Keyboard_keyWidth, parent.mDisplayWidth,
					parent.mDefaultWidth);
			defaultHeight = getDimensionOrFraction(a, R.styleable.Keyboard_keyHeight, parent.mDisplayHeight,
					parent.mDefaultHeight);
			defaultHorizontalGap = getDimensionOrFraction(a, R.styleable.Keyboard_horizontalGap, parent.mDisplayWidth,
					parent.mDefaultHorizontalGap);
			verticalGap = getDimensionOrFraction(a, R.styleable.Keyboard_verticalGap, parent.mDisplayHeight,
	...
	public static class Key {
		...
		private final static int[] KEY_STATE_NORMAL_ON = { android.R.attr.state_checkable,
				android.R.attr.state_checked };

		private final static int[] KEY_STATE_PRESSED_ON = { android.R.attr.state_pressed,
		...
		private final static int[] KEY_STATE_PRESSED_OFF = { android.R.attr.state_pressed,
		...
		public Key(final Resources res, final Row parent, final int x, final int y, final XmlResourceParser parser) {
			...
			width = getDimensionOrFraction(a, R.styleable.Keyboard_keyWidth, keyboard.mDisplayWidth,
					parent.defaultWidth);
			height = getDimensionOrFraction(a, R.styleable.Keyboard_keyHeight, keyboard.mDisplayHeight,
					parent.defaultHeight);
			gap = getDimensionOrFraction(a, R.styleable.Keyboard_horizontalGap, keyboard.mDisplayWidth,
					parent.defaultHorizontalGap);
			a.recycle();
			a = res.obtainAttributes(Xml.asAttributeSet(parser), R.styleable.Keyboard_Key);
			this.x += gap;
			final TypedValue codesValue = new TypedValue();
		...
		int[] parseCSV(final String value) {
			int count = 0;
			int lastIndex = 0;
			if (value.length() > 0) {
				count++;
		 ...
		 */
		public boolean isInside(final int x, final int y, final int w, final int h) {
			if (this.x <= x + w && x <= this.x + this.width && this.y <= y + h && y <= this.y + this.height)
				return true;
			else
				return false;
		}
		...
		public int squaredDistanceFrom(final int x, final int y) {
			...
			final int yDist = this.y + height / 2 - y;
	 ...
	 *            many keys as possible in each row.
	 */
	public Keyboard(final Context context, final int layoutTemplateResId, final CharSequence characters,
	...
	public int[] getNearestKeys(final int x, final int y) {
		...
		if (x >= 0 && x < getMinWidth() && y >= 0 && y < getHeight()) {
			final int index = y / mCellHeight * GRID_WIDTH + x / mCellWidth;
			if (index < GRID_SIZE)
	...
	private void loadKeyboard(final Context context, final XmlResourceParser parser) {
		boolean inKey = false;
		boolean inRow = false;
		final boolean leftMostKey = false;
		int row = 0;
		int x = 0;
		int y = 0;
		Key key = null;
	...
	private void skipToEndOfRow(final XmlResourceParser parser) throws XmlPullParserException, IOException {
		int event;
		while ((event = parser.next()) != XmlResourceParser.END_DOCUMENT)
	...
	private void parseKeyboardAttributes(final Resources res, final XmlResourceParser parser) {
		...
		mDefaultHeight = getDimensionOrFraction(a, R.styleable.Keyboard_keyHeight, mDisplayHeight, 75);
	...
	static int getDimensionOrFraction(final TypedArray a, final int index, final int base, final int defValue) {
