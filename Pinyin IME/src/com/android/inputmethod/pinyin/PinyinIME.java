...
public class PinyinIME extends InputMethodService {
	...
	private boolean processKey(final KeyEvent event, final boolean realAction) {
		...
		if (KeyEvent.KEYCODE_SPACE == keyCode && event.isShiftPressed()) {
			...
			final int allMetaState = KeyEvent.META_ALT_ON | KeyEvent.META_ALT_LEFT_ON | KeyEvent.META_ALT_RIGHT_ON
		...
		if (mInputModeSwitcher.isEnglishWithSkb())
			return mImEn.processKey(getCurrentInputConnection(), event, mInputModeSwitcher.isEnglishUpperCaseWithSkb(),
					realAction);
	...
	private boolean processFunctionKeys(final int keyCode, final boolean realAction) {
	...
	private boolean processStateIdle(final int keyChar, final int keyCode, final KeyEvent event,
			final boolean realAction) {
		...
		if (keyChar >= 'a' && keyChar <= 'z' && !event.isAltPressed()) {
			...
			final char fullwidth_char = KeyMapDream.getChineseLabel(keyCode);
			if (0 != fullwidth_char) {
				if (realAction) {
					final String result = String.valueOf(fullwidth_char);
			...
			if (realAction)
				if (keyChar == ',' || keyChar == '.')
					...
					final String result = String.valueOf((char) keyChar);
					commitResultText(result);
				}
			return true;
		}
	...
	private boolean processStateInput(int keyChar, final int keyCode, final KeyEvent event, final boolean realAction) {
		...
		if (keyChar >= 'a' && keyChar <= 'z' || keyChar == '\'' && !mDecInfo.charBeforeCursorIsSeparator()
			...
			if (!realAction)
				return true;
			inputCommaPeriod(mDecInfo.getCurrentFullSent(mCandidatesContainer.getActiveCandiatePos()), keyChar, true,
					ImeState.STATE_IDLE);
			return true;
			...
			final int currentPage = mCandidatesContainer.getCurrentPage();
	...
	private boolean processStatePredict(final int keyChar, final int keyCode, final KeyEvent event,
			final boolean realAction) {
		if (!realAction)
			return true;
	...
	private boolean processStateEditComposing(int keyChar, final int keyCode, final KeyEvent event,
			final boolean realAction) {
		if (!realAction)
			return true;

		final ComposingView.ComposingStatus cmpsvStatus = mComposingView.getComposingStatus();
	...
	private boolean tryInputRawUnicode(final String str) {
		if (str.length() > 7)
			if (str.substring(0, 7).compareTo("unicode") == 0)
				try {
					...
					final int unicode = Integer.parseInt(digitStr, radix);
					if (unicode > 0) {
						final char low = (char) (unicode & 0x0000ffff);
						final char high = (char) ((unicode & 0xffff0000) >> 16);
	...
	private boolean processSurfaceChange(final int keyChar, final int keyCode) {
	...
	private void changeToStateComposing(final boolean updateUi) {
	...
	private void changeToStateInput(final boolean updateUi) {
	...
	private void simulateKeyEventDownUp(final int keyCode) {
	...
	private void commitResultText(final String resultText) {
		final InputConnection ic = getCurrentInputConnection();
		if (null != ic)
	...
	private void updateComposingText(final boolean visible) {
		if (!visible)
			mComposingView.setVisibility(View.INVISIBLE);
		else {
	...
	private void inputCommaPeriod(String preEdit, final int keyChar, final boolean dismissCandWindow,
			final ImeState nextState) {
		if (keyChar == ',')
			preEdit += '\uff0c';
		else if (keyChar == '.')
			preEdit += '\u3002';
	...
	private void resetToIdleState(final boolean resetInlineText) {
	...
	private void chooseAndUpdate(final int candId) {
		if (!mInputModeSwitcher.isChineseText()) {
			final String choice = mDecInfo.getCandidate(candId);
			if (null != choice)
		...
		if (mDecInfo.getComposingStr().length() > 0) {
			...
			if (candId >= 0 && mDecInfo.canDoPrediction()) {
				...
				if (Settings.getPrediction()) {
					final InputConnection ic = getCurrentInputConnection();
					if (null != ic) {
						final CharSequence cs = ic.getTextBeforeCursor(3, 0);
						if (null != cs)
				...
				if (ImeState.STATE_IDLE == mImeState) {
					if (mDecInfo.getSplStrDecodedLen() == 0)
						...
						changeToStateInput(true);
				} else if (mDecInfo.selectionFinished())
					changeToStateComposing(true);
				showCandidateWindow(true);
			}
		} else
	...
	public void responseSoftKeyEvent(final SoftKey sKey) {
		if (null == sKey)
			return;
		...
		final int keyCode = sKey.getKeyCode();
		...
		if (sKey.isUserDefKey()) {
			...
			if (sKey.isKeyCodeKey()) {
				final KeyEvent eDown = new KeyEvent(0, 0, KeyEvent.ACTION_DOWN, keyCode, 0, 0, 0, 0,
						KeyEvent.FLAG_SOFT_KEYBOARD);
				final KeyEvent eUp = new KeyEvent(0, 0, KeyEvent.ACTION_UP, keyCode, 0, 0, 0, 0,
						KeyEvent.FLAG_SOFT_KEYBOARD);
				...
				boolean kUsed = false;
				final String keyLabel = sKey.getKeyLabel();
	...
	private void showCandidateWindow(final boolean showComposingView) {
		if (mEnvironment.needDebug())
	...
	private void updateIcon(final int iconId) {
		if (iconId > 0)
	...
	public void onStartInput(final EditorInfo editorInfo, final boolean restarting) {
		if (mEnvironment.needDebug())
			Log.d(TAG, "onStartInput " + " ccontentType: " + String.valueOf(editorInfo.inputType) + " Restarting:"
	...
	public void onStartInputView(final EditorInfo editorInfo, final boolean restarting) {
		if (mEnvironment.needDebug())
			Log.d(TAG, "onStartInputView " + " contentType: " + String.valueOf(editorInfo.inputType) + " Restarting:"
					+ String.valueOf(restarting));
	...
	public void onDisplayCompletions(final CompletionInfo[] completions) {
		...
		if (!mInputModeSwitcher.isChineseText() || ImeState.STATE_IDLE == mImeState
	...
	private void onChoiceTouched(final int activeCandNo) {
		if (mImeState == ImeState.STATE_COMPOSING)
			...
			if (null != mDecInfo.mAppCompletions && activeCandNo >= 0
				...
				final CompletionInfo ci = mDecInfo.mAppCompletions[activeCandNo];
				if (null != ci) {
					final InputConnection ic = getCurrentInputConnection();
	...
	public void showOptionsMenu() {
		...
		final CharSequence itemSettings = getString(R.string.ime_settings_activity_name);
		final CharSequence itemInputMethod = getString(R.string.ime_name);// com.android.internal.R.string.inputMethod
	...
	private class PopupTimer extends Handler implements Runnable {
		...
		public void run() {
			...
			if (!mFloatingWindow.isShowing())
				mFloatingWindow.showAtLocation(mCandidatesContainer, Gravity.LEFT | Gravity.TOP, mParentLocation[0],
	...
	public class ChoiceNotifier extends Handler implements CandidateViewListener {
		...
		ChoiceNotifier(final PinyinIME ime) {
			mIme = ime;
		}

		@Override
		public void onClickChoice(final int choiceId) {
			if (choiceId >= 0)
	...
	public class OnGestureListener extends GestureDetector.SimpleOnGestureListener {
		...
		public OnGestureListener(final boolean reponseGestures) {
		...
		public boolean onScroll(final MotionEvent e1, final MotionEvent e2, final float distanceX,
			...
			final long timeNow = e2.getEventTime();
			...
			final float vXTotal = (e2.getX() - e1.getX()) / spanTotal;
			final float vYTotal = (e2.getY() - e1.getY()) / spanTotal;

			// The distances are from the current point to the previous one.
			final float vXThis = -distanceX / spanThis;
			final float vYThis = -distanceY / spanThis;

			final float kX = vXTotal * vXThis;
			final float kY = vYTotal * vYThis;
			final float k1 = kX + kY;
			final float k2 = Math.abs(kX) + Math.abs(kY);

			if (k1 / k2 < 0.8) {
				mNotGesture = true;
				return false;
			}
			final float absVXTotal = Math.abs(vXTotal);
			final float absVYTotal = Math.abs(vYTotal);
		...
		@Override
		public boolean onFling(final MotionEvent me1, final MotionEvent me2, final float velocityX,
				final float velocityY) {
			return mGestureRecognized;
		}

		public void onDirectionGesture(final int gravity) {
	...
	public class DecodingInfo {
		...
		public void addSplChar(final char ch, final boolean reset) {
			if (reset) {
				mSurface.delete(0, mSurface.length());
				mSurfaceDecodedLen = 0;
				mCursorPos = 0;
		...
		public String getCurrentFullSent(final int activeCandPos) {
			try {
		...
		private void chooseDecodingCandidate(final int candId) {
		...
		private void updateDecInfoForSearch(final int totalChoicesNum) {
		...
		private void choosePredictChoice(final int choiceId) {
			if (ImeState.STATE_PREDICT != mImeState || choiceId < 0 || choiceId >= mTotalChoicesNum)
				return;

			final String tmp = mCandidatesList.get(choiceId);
		...
		public String getCandidate(final int candId) {
		...
		private void getCandiagtesForCache() {
			final int fetchStart = mCandidatesList.size();
				...
				if (ImeState.STATE_INPUT == mImeState || ImeState.STATE_IDLE == mImeState
					...
					if (null != mAppCompletions)
						for (int pos = fetchStart; pos < fetchSize; pos++) {
							final CompletionInfo ci = mAppCompletions[pos];
							if (null != ci) {
								final CharSequence s = ci.getText();
								if (null != s)
		...
		public boolean pageReady(final int pageNo) {
		...
		public boolean preparePage(final int pageNo) {
			// If the page number is less than 0, return false
			if (pageNo < 0)
				return false;
		...
		public void preparePredicts(final CharSequence history) {
			if (null == history)
				return;
			...
			if (Settings.getPrediction()) {
				final String preEdit = history.toString();
				final int predictNum = 0;
				if (null != preEdit)
					try {
		...
		private void prepareAppCompletions(final CompletionInfo completions[]) {
			resetCandidates();
		...
		public boolean pageBackwardable(final int currentPage) {
			if (currentPage > 0)
				return true;
			return false;
		}
		...
		public void moveCursorToEdge(final boolean left) {
			if (left)
				mCursorPos = 0;
			else
				mCursorPos = mSurface.length();
