...
public class SkbContainer extends RelativeLayout implements OnTouchListener {
	...
	public SkbContainer(final Context context, final AttributeSet attrs) {
		super(context, attrs);
	...
	public void setService(final InputMethodService service) {
		mService = service;
	}

	public void setInputModeSwitcher(final InputModeSwitcher inputModeSwitcher) {
	...
	public boolean isCurrentSkbSticky() {
		if (null == mMajorView)
			return true;
		final SoftKeyboard skb = mMajorView.getSoftKeyboard();
		if (null != skb)
			return skb.getStickyFlag();
		return true;
	}

	public void toggleCandidateMode(final boolean candidatesShowing) {
		if (null == mMajorView || !mInputModeSwitcher.isChineseText() || mLastCandidatesShowing == candidatesShowing)
			return;
		...
		final int state = mInputModeSwitcher.getTooggleStateForCnCand();
	...
	public void updateInputMode() {
		final int skbLayout = mInputModeSwitcher.getSkbLayout();
	...
	private void updateSkbLayout() {
		final int screenWidth = mEnvironment.getScreenWidth();
		final int keyHeight = mEnvironment.getKeyHeight();
		final int skbHeight = mEnvironment.getSkbHeight();
		...
		final SkbPool skbPool = SkbPool.getInstance();

		switch (mSkbLayout) {
		case R.xml.skb_qwerty:
			majorSkb = skbPool.getSoftKeyboard(R.xml.skb_qwerty, R.xml.skb_qwerty, screenWidth, skbHeight,
					getContext());
			break;

		case R.xml.skb_sym1:
			majorSkb = skbPool.getSoftKeyboard(R.xml.skb_sym1, R.xml.skb_sym1, screenWidth, skbHeight, getContext());
			break;

		case R.xml.skb_sym2:
			majorSkb = skbPool.getSoftKeyboard(R.xml.skb_sym2, R.xml.skb_sym2, screenWidth, skbHeight, getContext());
			break;

		case R.xml.skb_smiley:
			majorSkb = skbPool.getSoftKeyboard(R.xml.skb_smiley, R.xml.skb_smiley, screenWidth, skbHeight,
					getContext());
			break;

		case R.xml.skb_phone:
			majorSkb = skbPool.getSoftKeyboard(R.xml.skb_phone, R.xml.skb_phone, screenWidth, skbHeight, getContext());
			break;
		default:
		}
	...
	private void responseKeyEvent(final SoftKey sKey) {
		if (null == sKey)
			return;
	...
	private SoftKeyboardView inKeyboardView(final int x, final int y, final int positionInParent[]) {
		if (mPopupSkbShow) {
			if (mPopupX <= x && mPopupX + mPopupSkb.getWidth() > x && mPopupY <= y
	...
	private void popupSymbols() {
		final int popupResId = mSoftKeyDown.getPopupResId();
		if (popupResId > 0) {
			final int skbContainerWidth = getWidth();
			final int skbContainerHeight = getHeight();
			// The paddings of the background are not included.
			final int miniSkbWidth = (int) (skbContainerWidth * 0.8);
			final int miniSkbHeight = (int) (skbContainerHeight * 0.23);

			final SkbPool skbPool = SkbPool.getInstance();
			final SoftKeyboard skb = skbPool.getSoftKeyboard(popupResId, popupResId, miniSkbWidth, miniSkbHeight,
					getContext());
			if (null == skb)
				return;
			...
			mPopupSkb
					.setWidth(skb.getSkbCoreWidth() + mPopupSkbView.getPaddingLeft() + mPopupSkbView.getPaddingRight());
			mPopupSkb.setHeight(
					skb.getSkbCoreHeight() + mPopupSkbView.getPaddingTop() + mPopupSkbView.getPaddingBottom());
	...
	private void dimSoftKeyboard(final boolean dimSkb) {
	...
	private void resetKeyPress(final long delay) {
	...
	public boolean handleBack(final boolean realAction) {
		if (mPopupSkbShow) {
			if (!realAction)
				return true;
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		final int measuredWidth = env.getScreenWidth();
	...
	public boolean onTouchEvent(final MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (null != mSkv)
				mSoftKeyDown = mSkv.onKeyPress(x - mSkvPosInContainer[0], y - mSkvPosInContainer[1], mLongPressTimer,
						false);
			break;
		...
		case MotionEvent.ACTION_MOVE:
			...
			final SoftKeyboardView skv = inKeyboardView(x, y, mSkvPosInContainer);
			if (null != skv)
				if (skv != mSkv) {
					mSkv = skv;
	...
	public boolean onTouch(final View v, final MotionEvent event) {
		...
		final MotionEvent newEv = MotionEvent.obtain(event.getDownTime(), event.getEventTime(), event.getAction(),
				...
				event.getEdgeFlags());
		final boolean ret = onTouchEvent(newEv);
		return ret;
	}
	...
	class LongPressTimer extends Handler implements Runnable {
		...
		public LongPressTimer(final SkbContainer skbContainer) {
